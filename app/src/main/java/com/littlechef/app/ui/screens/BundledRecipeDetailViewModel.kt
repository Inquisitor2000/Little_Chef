...
import com.littlechef.app.domain.usecase.CreateMealPlanUseCase
...
class BundledRecipeDetailViewModel @Inject constructor(
    ...
    private val groceryRepository: com.littlechef.app.domain.repository.GroceryRepository,
    ...
    private val startCookingUseCase: com.littlechef.app.domain.usecase.StartCookingUseCase,
    private val mealRepository: com.littlechef.app.domain.repository.MealRepository,
    private val mealPlanRepository: com.littlechef.app.domain.repository.MealPlanRepository,
    ...
    private suspend fun lookupIngredientByEnglishName(englishName: String): com.littlechef.app.domain.model.Ingredient? {
        ...
        if (matchResult != null) {
            ...
            val ingredient = com.littlechef.app.domain.model.Ingredient(
    ...
    fun loadRecipe(cuisine: Cuisine, recipeId: String) {
        viewModelScope.launch {
            ...
            _recipe.value?.let { recipe ->
                val allergensSet = mutableSetOf<com.littlechef.app.domain.model.Allergen>()
     ...
     */
    private fun findAllergensFromCatalog(ingredientName: String): List<com.littlechef.app.domain.model.Allergen> {
    ...
    fun translateInstructions(instructions: String): String {
        return com.littlechef.app.domain.util.InstructionTranslator.translateInstructions(
            instructions,
            translationSystem
        )
    }
        ...
        viewModelScope.launch {
            ...
            if (result is CheckRecipeIngredientsUseCase.Result.MissingIngredients && 
                ...
                result.missing.forEach { missing ->
                    ...
                    val newItem = com.littlechef.app.domain.model.GroceryItem(
        ...
        viewModelScope.launch {
            ...
            if (result is CheckRecipeIngredientsUseCase.Result.MissingIngredients && 
                ...
                val groceryItems = result.missing.map { missing ->
                    ...
                    com.littlechef.app.domain.model.GroceryItem(
                        id = java.util.UUID.randomUUID().toString(),
                        ingredientName = missing.name, // Store English name for translation
                        ingredientId = ingredient?.id, // Store ingredient ID for category lookup
                        quantity = missing.required - missing.available,
    ...
    fun planMeal(mealType: com.littlechef.app.domain.model.MealType, plannedDate: Long) {
        viewModelScope.launch {
            val recipe = _recipe.value
            ...
            if (recipe != null) {
                ...
                val scrapedRecipe = com.littlechef.app.data.remote.ScrapedRecipe(
                    name = recipe.name,
                    ingredients = recipe.ingredients.map { 
                        com.littlechef.app.data.remote.ScrapedIngredient(
                            name = it.name,
                    ...
                    onSuccess = { meal ->
                        ...
                        mealRepository.updateMeal(
                            ...
                            fullMeal.ingredients.map { 
                                com.littlechef.app.domain.repository.MealIngredientInput(
                                    it.ingredient.id, 
                                    it.quantity,
                                    it.isStarIngredient
                                )
                    ...
                    mealResult.fold(
                        onSuccess = { meal ->
                            ...
                            val mealPlanResult = createMealPlanUseCase(
                                ...
                                mealType = com.littlechef.app.domain.model.MealType.DINNER
                            )
                            ...
                            when (mealPlanResult) {
                                is CreateMealPlanUseCase.Result.Success -> {
                                    ...
                                    when (cookingResult) {
                                        is com.littlechef.app.domain.usecase.StartCookingUseCase.Result.Success -> {
                                            _madeMealResult.value = MadeMealResult(
                                                success = true,
                                                message = localeContext.getString(R.string.recipe_started_cooking)
                                            )
                                        }
                                        is com.littlechef.app.domain.usecase.StartCookingUseCase.Result.InsufficientIngredients -> {
                                            _madeMealResult.value = MadeMealResult(
                                                success = false,
                                        ...
                                        is com.littlechef.app.domain.usecase.StartCookingUseCase.Result.Error -> {
                                            _madeMealResult.value = MadeMealResult(
                                                success = false,
            ...
            if (recipe != null) {
                try {
                    ...
                    val deductibleIngredients = recipe.ingredients.filter { ingredient ->
                        com.littlechef.app.domain.model.NonDeductibleIngredients.shouldDeduct(
                            ingredient.name,
                            ingredient.unit
                        )
                    }
                    ...
                    val transactions = mutableListOf<com.littlechef.app.domain.model.InventoryTransaction>()
                    ...
                    for (ingredient in deductibleIngredients) {
                        ...
                        if (ingredientEntity != null) {
                            ...
                            if (available >= adjustedQuantity) {
                                ...
                                val transaction = com.littlechef.app.domain.model.InventoryTransaction(
                                    id = java.util.UUID.randomUUID().toString(),
                                    ingredientId = ingredientEntity.id,
                                    quantityChange = -adjustedQuantity,
                                    status = com.littlechef.app.domain.model.TransactionStatus.COMMITTED,
    ...
    private suspend fun findIngredientByName(name: String): com.littlechef.app.domain.model.Ingredient? {
