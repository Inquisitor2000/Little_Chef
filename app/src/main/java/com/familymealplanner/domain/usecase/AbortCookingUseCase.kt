...
class AbortCookingUseCase @Inject constructor(
    ...
    suspend operator fun invoke(
        ...
        // Check if meal plan is in correct state (can cancel PLANNED or abort COOKING)
        if (mealPlan.status != MealPlanStatus.COOKING && mealPlan.status != MealPlanStatus.PLANNED) {
            return Result.Error("Meal plan must be in PLANNED or COOKING status to cancel")
        }
        ...
        // Update all reservation transactions to RELEASED
        ...
        // Update meal plan status to ABORTED
        val updatedMealPlan = mealPlan.copy(
            status = MealPlanStatus.ABORTED,
            completedAt = now,
            updatedAt = now
        )
